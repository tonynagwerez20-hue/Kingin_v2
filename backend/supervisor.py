...
_handler = RotatingFileHandler(str(log_file), maxBytes=5 * 1024 * 1024, backupCount=3)
_stream  = logging.StreamHandler()
logging.basicConfig(
    handlers=[_handler, _stream],
    level=logging.INFO,
...
def main():
    ...
    # Trading Engine — correct path
    supervisor.add_component(
        "Trading Engine",
        PROJECT_ROOT / "Engine" / "main_loop.py",
    )

    # KingIn API Server
    supervisor.add_component(
        "KingIn API",
        PROJECT_ROOT / "kingin_api.py",
    )
